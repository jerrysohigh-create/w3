(function () {
  ...
  function renderWinners(rows) {
    ...
    if (!winners.length) {
      ...
      setText("s2-winners-count", winners.length.toLocaleString("en-US") + " public winning records; the page is desensitized and the link retains its complete address.");
    }
    if (previous) previous.disabled = currentWinnerPage <= 1;
    if (next) next.disabled = currentWinnerPage >= totalPages || !winners.length;
    setText("s2-page-indicator", "PAGE " + currentWinnerPage + " / " + totalPages + " · " + winners.length.toLocaleString("en-US") + " RECORDS");
  }
