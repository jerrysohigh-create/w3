(function () {
  ...
  function render(payload, fallback) {
    ...
    if (countMeta) countMeta.textContent = decimal(rate, 2) + "% / " + number(total) + " 個 OG 錢包 · BSC 區塊 " + number(block);
    if (allocatedNode && Number.isFinite(allocated)) allocatedNode.textContent = decimal(allocated, 6) + " MS2";
    if (allocatedMeta) allocatedMeta.textContent = "TOTALALLOCATED() · " + (fallback || payload._meta?.lastError || Date.now() - Date.parse(payload._meta?.fetchedAt) > 120000 ? "快照 / 更新延遲" : "鏈上已同步") + " · " + (payload._meta?.fetchedAt || "—");
    return true;
  }
