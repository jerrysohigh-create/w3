(function () {
  ...
  function renderEvents(container, limit) {
    ...
    container.innerHTML = items.map(function (item, index) {
      ...
      var images = item.images.map(function (path, imageIndex) {
        return '<a href="' + assetBase + path + "\" target=\"_blank\" rel=\"noopener noreferrer\" aria-label=\"檢視" + item.title + "照片" + (imageIndex + 1) + '"><img src="' + assetBase + path + '" alt="' + item.title + "照片" + (imageIndex + 1) + '" loading="lazy"></a>';
      }).join("");
      ...
      var action = item.article
        ...
        : (item.source
          ? '<a href="' + item.source + "\" target=\"_blank\" rel=\"noopener noreferrer\" aria-label=\"在新視窗中開啟" + item.title + ' 的官方來源">SOURCE RECORD ↗</a>'
      ...
      return '<article class="nr-event-row" id="' + escapeHtml(item.slug) + '"><header><time class="nr-event-date" datetime="' + isoDate + '"><span>0' + (index + 1) + '</span><b>' + year + '</b><em>' + monthDay + '</em><small>' + datePrecision + '</small></time><div><p>LOCATION / ' + item.place + '</p><h2>' + item.title + '</h2><small>' + item.note + '</small></div>' + action + '</header>' +
        ...
        '<div class="nr-event-strip" tabindex="0" aria-label="' + item.title + "自動捲動照片，可手動橫向捲動\" style=\"--nr-scroll-duration:" + Math.max(18, item.images.length * 6) + 's"><div class="nr-event-track"><div class="nr-event-group">' + images + '</div><div class="nr-event-group" aria-hidden="true">' + duplicateImages + '</div></div></div></article>';
    }).join("");
  }
  ...
  function sourceRow(record) {
    ...
    return '<article class="nr-source-row ' + status.className + '" data-source-status="' + status.group + '" data-source-commentary="' + (record.source_label === "Independent Commentary" ? "true" : "false") + '" data-source-search="' + escapeHtml((record.title + " " + record.outlet + " " + record.cluster + " " + record.source_label).toLowerCase()) + '">' +
      ...
      '<a href="' + escapeHtml(record.url) + "\" target=\"_blank\" rel=\"noopener noreferrer\" aria-label=\"在新視窗開啟來源：" + escapeHtml(record.title) + '">SOURCE ↗</a>' +
      '</article>';
  }
  ...
  function verificationStatus(item) {
    ...
    if (type === "dual-post") return { group: "bilateral", label: "BILATERAL / DUAL POST", detail: "雙方各自 X 原文已鎖定", className: "is-dual" };
    if (type === "dual-capital") return { group: "bilateral", label: "BILATERAL / CAPITAL", detail: "雙方官方帖確認資本關係", className: "is-dual" };
    if (type === "reply-confirmed") return { group: "bilateral", label: "BILATERAL / REPLY", detail: "單方發文 + 對方官方確認", className: "is-dual" };
    if (type === "repost-confirmed") return { group: "bilateral", label: "BILATERAL / REPOST", detail: "單方發文 + 對方官方轉發", className: "is-dual" };
    ...
    if (type === "bilateral-confirmed-pending") return { group: "bilateral", label: "BILATERAL / OFFICIAL CONFIRMATION", detail: "雙方官方確認成立；部分 status ID 待回收", className: "is-dual" };
    if (type === "reply-confirmed-pending") return { group: "bilateral", label: "BILATERAL / OFFICIAL RESPONSE", detail: "合作宣告 + 對方官方回應；status ID 待回收", className: "is-dual" };
    if (type === "repost-confirmed-pending") return { group: "bilateral", label: "BILATERAL / OFFICIAL REPOST", detail: "合作公告 + 對方官方轉發；status ID 待回收", className: "is-dual" };
    if (type === "joint-activation-confirmed") return { group: "bilateral", label: "BILATERAL / JOINT ACTIVATION", detail: "雙方聯合活動與有效官方帳號已確認", className: "is-dual" };
    if (type === "platform-listing-confirmed") return { group: "bilateral", label: "BILATERAL / PLATFORM LISTING", detail: "MAGNE 原文版 + 官方平臺專案收錄", className: "is-dual" };
    if (type === "capital-magne") return { group: "single", label: "ONE X URL / CAPITAL", detail: "MAGNE 公告的策略性投資關係", className: "is-magne" };
    if (type === "single-magne") return { group: "single", label: "ONE X URL / MAGNE", detail: "MAGNE 原文已鎖定；對方動作未索引", className: "is-magne" };
    if (type === "single-partner") return { group: "single", label: "ONE X URL / PARTNER", detail: "合作方原帖已鎖定；MAGNE 動作未索引", className: "is-partner" };
    if (type === "source-conflict") return { group: "coverage", label: "SOURCE CONFLICT", detail: "原始連結與物件不一致", className: "is-conflict" };
    if (type === "source-inaccessible") return { group: "pending", label: "SOURCE INACCESSIBLE / RECHECK", detail: "歷史位址已保留；X 目前明確回傳不可用", className: "is-mirror" };
    if (type === "coverage-only") return { group: "coverage", label: "COVERAGE ONLY", detail: "僅作發現線索，不計雙方確認", className: "is-coverage" };
    if (type === "repost-pending") return { group: "pending", label: "REPOST OBSERVED / X PENDING", detail: "官方帳號已列出，原帖 ID 待回收", className: "is-mirror" };
    if (type === "reply-pending") return { group: "pending", label: "REPLY OBSERVED / X PENDING", detail: "官方帳號已列出，確認帖 ID 待回收", className: "is-mirror" };
    if (type === "dual-pending") return { group: "pending", label: "DUAL POSTS / X PENDING", detail: "雙方帳號已列出，原帖 ID 待回收", className: "is-mirror" };
    return { group: "pending", label: "POST ID PENDING", detail: "官方 X 位址已列出，原始貼文 status ID 待回收", className: "is-mirror" };
  }
  ...
  function renderEvidenceStats(container) {
    ...
    var stats = [
      [String(items.length).padStart(2, "0"), "TIMELINE RECORDS", "歷史基線 + 公開補錄"],
      [String(counts.bilateral || 0).padStart(2, "0"), "BILATERAL CONFIRMED", "雙方官方互動確認；部分 ID 待回收"],
      [String(counts.single || 0).padStart(2, "0"), "ONE X URL", "已鎖定一則原文；不等於事實單邊"],
      [String(counts.pending || 0).padStart(2, "0"), "X ID PENDING", "已發現，待回收原帖"],
      [String(counts.coverage || 0).padStart(2, "0"), "COVERAGE / CONFLICT", "不計雙方確認"]
    ];
  ...
  function sourceLinks(item) {
    ...
    [
      ...
      var ariaType = isXStatus ? "的 X 原帖" : (isXProfile ? "的官方 X 帳號" : (isPlatform ? "的官方平臺專案頁" : "的獨立報道"));
      links.push('<a href="' + escapeHtml(url) + "\" target=\"_blank\" rel=\"noopener noreferrer\" aria-label=\"在新視窗中開啟" + escapeHtml(item.partner) + ariaType + '">' + escapeHtml(label) + '</a>');
    });
  ...
  function renderEvidenceActivities(container) {
    ...
    container.innerHTML = evidence.activities.map(function (item, index) {
      var replay = item.replayUrl ? '<a href="' + escapeHtml(item.replayUrl) + "\" target=\"_blank\" rel=\"noopener noreferrer\" aria-label=\"在新視窗中開啟" + escapeHtml(item.title) + ' 的 X Space">SPACE ↗</a>' : '';
      return '<article data-activity-format="' + escapeHtml(item.format) + '" data-activity-status="' + escapeHtml(item.status) + '" data-activity-search="' + escapeHtml((item.title + " " + item.publisher + " " + item.handle).toLowerCase()) + '">' +
        ...
        '<div class="nr-activity-links"><a href="' + escapeHtml(item.url) + "\" target=\"_blank\" rel=\"noopener noreferrer\" aria-label=\"在新視窗中開啟" + escapeHtml(item.title) + ' 的原始 X 記錄">SOURCE ↗</a>' + replay + '</div>' +
      '</article>';
    }).join("");
